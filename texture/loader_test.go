...
func TestLoadImage(t *testing.T) {
	img := texture.MustLoad("./testdata/ground.png", texture.WithCorrectGamma(true))
...
func BenchmarkLoadImage(b *testing.B) {
	b.Run("without-correction", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			texture.MustLoad("../testdata/ground.png", texture.WithCorrectGamma(false))
		}
	})
	b.Run("with-correction", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			texture.MustLoad("../testdata/ground.png", texture.WithCorrectGamma(true))
		}
	})
}
