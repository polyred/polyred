...
| [gpu-by-default.md](foundations/gpu-by-default.md) | **Drafted (small)** | flip the default to GPU: `NewRenderer` auto-opens a device (CPU fallback), add `render.CPU()`; renderer owns/closes a device it opened. Per-pass fallback keeps it safe |
