...
func downloadDLLs() (path string, err error) {
	...
	log.Printf("DLLs could not be written to %s", appdataPath())
	...
	log.Printf("DLLs could not be written to GOPATH")
...
func findDLLs() (err error) {
	load := func(path string) (bool, error) {
		if path != "" {
			...
			gles.LibGLESv2.Name = filepath.Join(path, filepath.Base(gles.LibGLESv2.Name))
			libEGL.Name = filepath.Join(path, filepath.Base(libEGL.Name))
		}

		if err := gles.LibGLESv2.Load(); err == nil {
			...
			if path == "" {
				log.Printf("DLLs found")
			} else {
				log.Printf("DLLs found in: %q", path)
			}
			return true, nil
		}
	...
	log.Printf("DLLs written to %s", path)
