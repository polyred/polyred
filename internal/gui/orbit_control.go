...
func NewOrbitControl(w *Window, cam camera.Interface) *OrbitControl {
...
func (oc *OrbitControl) Pan(deltaX, deltaY float64) {
	...
	c := 2 * vdir.Len() * math.Tan(math.DegToRad(oc.cam.Fov()/2.0)) /
		math.Max(float64(oc.window.width), float64(oc.window.height))
...
func (oc *OrbitControl) OnCursor(evname EventName, ev Event) {
	...
	switch oc.state {
	case stateRotate:
		c := -2 * math.Pi * oc.RotSpeed / math.Max(float64(oc.window.width), float64(oc.window.height))
