...
func (m *TriangleSoup) Normalize() {
	...
	for i := 0; i < len(m.faces); i++ {
		f := m.faces[i]
		f.V[0].Pos = f.V[0].Pos.Apply(m.ModelMatrix()).Translate(-center.X, -center.Y, -center.Z).Scale(fac, fac, fac, 1)
		f.V[1].Pos = f.V[1].Pos.Apply(m.ModelMatrix()).Translate(-center.X, -center.Y, -center.Z).Scale(fac, fac, fac, 1)
		f.V[2].Pos = f.V[2].Pos.Apply(m.ModelMatrix()).Translate(-center.X, -center.Y, -center.Z).Scale(fac, fac, fac, 1)
	}
